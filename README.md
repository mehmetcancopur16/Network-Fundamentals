# 🌐 Network Fundamentals for Cybersecurity

Welcome to the **Network Fundamentals** repository! 🚀

This comprehensive guide is designed from the ground up to teach you how networks operate, which is the absolute foundation of Cybersecurity. You cannot protect (or hack) a system if you don't understand how data travels through it. Whether you are a beginner or looking to refresh your knowledge, this repo will serve as your ultimate reference guide.

## 📖 Table of Contents

1. [Network Models and Encapsulation](01-Network-Models-and-Encapsulation.md)
2. [Addressing and Subnetting](02-Addressing-and-Subnetting.md)
3. [Transport Protocols and Ports](03-Transport-Protocols-and-Ports.md)
4. [Layer 2 Switching and VLANs](04-Layer-2-Switching-and-VLANs.md)
5. [Network Routing and NAT](05-Network-Routing-and-NAT.md)
6. [Physical Layer and Cabling](06-Physical-Layer-and-Cabling.md)
7. [Troubleshooting Tools](07-Troubleshooting-Tools.md)
8. [Glossary of Networking Terms](Glossary.md) _(Coming Soon)_

---

_Created and maintained with passion for the cybersecurity community by [Mehmet Can Çopur](https://github.com/mehmetcancopur16)._
